...
export const hostsService = {
  ...
  async updateHostWithFile(
    ...
    // - If no file: append empty string to clear the image
    if (profileImageFile) {
      formData.append('profile_image', profileImageFile)
    } else {
      // Send empty value to clear the image
...
export const paymentMethodsService = {
  ...
  async deletePaymentMethod(eventId: string, paymentMethodId: number): Promise<ApiResponse<void>> {
    return apiService.delete(`/api/events/${eventId}/payment-methods/${paymentMethodId}/`)
  },
