<template>
  <div class="bg-white rounded-2xl shadow-sm border border-slate-200 overflow-hidden">
    <!-- QR Scanner takes over the card when active so the camera viewport gets
         the full width on mobile. The QRCodeScanner's built-in Cancel button
         emits `close`, which flips us back to the manual input. -->
    <div v-if="mode === 'qr'" class="p-3 sm:p-4">
      <QRCodeScanner
        :continuous="true"
        ...
        @close="setMode('manual')"
      />
    </div>

    <!-- Default surface: manual code entry with an inline camera icon to jump
         into QR mode. Mirrors RegistrationCheckinModal so staff get a single
         consistent check-in pattern across both flows. -->
    <form
      v-else
      class="p-4 space-y-4"
      @submit.prevent="onManualSubmit"
    >
      <div class="bg-emerald-50 border border-emerald-200 rounded-xl p-3 text-xs text-slate-600 flex items-start gap-2">
      ...
      <div>
        ...
        <div class="relative">
          <input
            ...
            spellcheck="false"
            maxlength="12"
            class="w-full pl-4 pr-14 py-3 text-center text-lg font-mono tracking-[0.2em] uppercase border border-slate-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-emerald-500/20 focus:border-emerald-400 hover:border-emerald-300 transition-colors disabled:bg-slate-50 disabled:text-slate-400"
          ...
          <button
            type="button"
            class="absolute right-1.5 top-1/2 -translate-y-1/2 p-2.5 text-slate-600 hover:text-emerald-600 transition-colors rounded-lg hover:bg-slate-100 active:bg-slate-200 focus:outline-none focus:ring-2 focus:ring-emerald-500/30 disabled:opacity-40 disabled:cursor-not-allowed"
            :aria-label="t('management.tickets.scan.modes.qr')"
            :title="t('management.tickets.scan.modes.qr')"
            :disabled="disabled"
            @click="setMode('qr')"
          >
...
import { AlertCircle, Camera, UserCheck } from 'lucide-vue-next'
...
// after returning from the QR scanner.
watch(
  () => props.mode,
  (next) => {
    if (next === 'manual') {
