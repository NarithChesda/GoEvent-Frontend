        ...
        <div class="flex min-h-full items-center justify-center p-4">
          <div class="relative bg-white rounded-3xl shadow-2xl p-4 sm:p-6 w-full max-w-md" @click.stop>
            <div class="flex items-start justify-between gap-3 mb-4">
              <div>
                <h3 class="text-base font-semibold text-slate-900">
                  {{ t('management.showcasePreview.editors.youtubeTitle') }}
                </h3>
                <p class="text-xs sm:text-sm text-slate-500 mt-1">
                  {{ t('management.showcasePreview.editors.youtubeDescription') }}
                </p>
              </div>
              <button
                type="button"
            ...
            <textarea
              ...
              "
              :placeholder="t('management.showcasePreview.editors.youtubePlaceholder')"
            ></textarea>
            <p v-if="showInvalid" class="text-xs sm:text-sm text-red-600 mt-1">
              {{ t('management.showcasePreview.editors.youtubeInvalid') }}
            </p>
...
const previewUrl = computed(() => extractYouTubeEmbedUrl(input.value) || '')
const showInvalid = computed(() => input.value.trim() !== '' && !previewUrl.value)

const close = () => emit('update:modelValue', false)

// Empty input clears the video (same null semantics as EmbedsSection's remove).
const save = async () => {
  saving.value = true
  try {
    const response = await eventsService.patchEvent(props.eventId, {
      youtube_embed_link: previewUrl.value || null,
    })
