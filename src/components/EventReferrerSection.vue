<template>
  <div class="bg-white/80 backdrop-blur-sm border border-white/20 rounded-3xl shadow-xl p-6">
    ...
    <div v-if="referrerDetails && !isEditing" class="mb-4">
      <div class="p-4 bg-gradient-to-br from-blue-50 to-purple-50 rounded-2xl border border-blue-200/50">
      ...
      <form @submit.prevent="saveReferrer" class="space-y-4">
        <div>
          ...
          <div class="relative">
            ...
            />
            <Mail class="w-5 h-5 text-slate-400 absolute left-3 top-1/2 transform -translate-y-1/2" />
          </div>
    ...
    <div class="mt-6 p-4 bg-blue-50/50 rounded-xl border border-blue-200/50">
      <div class="flex items-start space-x-3">
        ...
        <div class="text-sm text-slate-700">
          ...
          <p>Referrers help promote your event and may receive special benefits or commissions based on your agreement.</p>
        </div>
      </div>
    </div>
    ...
    <Teleport to="body">
      <Transition name="modal">
        <div v-if="showRemoveConfirm" class="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm" @click="cancelRemove">
          <div 
          ...
          >
            <div class="p-6">
              ...
              <h3 class="text-lg font-semibold text-slate-900 text-center mb-2">Remove Referrer?</h3>
              <p class="text-sm text-slate-600 text-center mb-6">
                Are you sure you want to remove <span class="font-medium">{{ referrerDetails?.first_name }} {{ referrerDetails?.last_name }}</span> as the referrer for this event?
              </p>
              <div class="flex space-x-3">
                <button
                  @click="cancelRemove"
...
const saveReferrer = async () => {
  ...
  if (props.organizerEmail && referrerEmail.value.toLowerCase() === props.organizerEmail.toLowerCase()) {
  ...
  try {
    ...
    const apiError = err as { response?: { data?: { referrer_email?: string[], detail?: string } }, message?: string }
