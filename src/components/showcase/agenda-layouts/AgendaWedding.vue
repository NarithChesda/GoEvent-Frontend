...
// inline "change this day's date" edit (requestDateChange below) moves every
// item in the currently-viewed group to a new date string. Without this,
// activeTab kept pointing at the OLD date, which no longer exists in
// agendaTabs/agendaByDate after the refetch, so the moved items simply
// vanished from view until the page was hard-refreshed (remounting re-ran
// what used to be a one-time onMounted pick). `immediate: true` also covers
// that original initial-pick job, so onMounted no longer needs to.
watch(
  agendaTabs,
  (tabs) => {
