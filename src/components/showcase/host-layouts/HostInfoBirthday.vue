<template>
  <div class="host-info-birthday" :class="{ 'khmer-text': currentLanguage === 'kh' }">
    ...
    <div v-if="hosts.length > 0" class="text-center space-y-1 sm:space-y-1.5 px-4">
      ...
      >
        <span
          v-for="(word, index) in splitToWords(hosts[0]?.title || 'Birthday Star')"
          :key="`title-${currentLanguage}-${index}`"
          class="bounce-word"
          :style="{ animationDelay: `${animationDelays.title + index * WORD_DELAY}s` }"
        >{{ word }}{{ index < splitToWords(hosts[0]?.title || 'Birthday Star').length - 1 ? '\u00A0' : '' }}</span>
      </p>
...
const animationDelays = computed(() => {
  ...
  const welcome = getNextDelay(props.welcomeMessage || 'Happy Birthday!')
  ...
  currentDelay += 0.25
  const title = getNextDelay(props.hosts[0]?.title || 'Birthday Star')
