        ...
        <form class="space-y-4 sm:space-y-6" @submit.prevent="handleSignUp">
          ...
          <div class="mt-6">
            ...
            <div class="mt-4 sm:mt-6 space-y-3">
              ...
              <div class="telegram-button-container relative">
                <!-- Custom styled button overlay -->
                <div
                  v-if="!isRegisteredDomain"
                  @click="handleTelegramButtonClick"
                  class="w-full flex items-center justify-center px-4 py-3 border border-slate-200 rounded-xl bg-white/50 backdrop-blur-sm hover:bg-white/80 transition-all duration-200 transform hover:scale-105 focus:outline-none focus:ring-2 focus:ring-[#1e90ff] cursor-pointer group"
                >
                ...
                <!-- On registered domains: overlay styled button on top of real Telegram widget -->
                <div v-else class="telegram-widget-wrapper">
                  <div class="telegram-overlay">
                    <svg class="h-5 w-5 mr-2" viewBox="0 0 240 240" xmlns="http://www.w3.org/2000/svg">
                      <defs>
                        <linearGradient id="telegram-gradient-2" x1="0%" y1="0%" x2="100%" y2="100%">
                          <stop offset="0%" style="stop-color:#2AABEE" />
                          <stop offset="100%" style="stop-color:#229ED9" />
                        </linearGradient>
                      </defs>
                      <circle cx="120" cy="120" r="120" fill="url(#telegram-gradient-2)" />
                      <path
                        fill="#fff"
                        d="M81.229 128.772l14.237 39.406s1.78 3.687 3.686 3.687 30.255-29.492 30.255-29.492l31.525-60.89L81.737 118.6z"
                      />
                  ...
                  <div id="telegram-login-widget" class="telegram-widget-hidden"></div>
                </div>
              </div>
            </div>
          </div>
...
const handleTelegramButtonClick = () => {
  // Only show error message on localhost/unregistered domains
  errorMessage.value =
...
/* Telegram widget wrapper for registered domains */
...
/* Styled overlay button that sits on top of Telegram widget */
.telegram-overlay {
  position: absolute;
  top: 0;
  left: 0;
  ...
  pointer-events: none; /* Allow clicks to pass through to widget below */
}

.telegram-widget-wrapper:hover .telegram-overlay {
  background: rgba(255, 255, 255, 0.8);
  transform: scale(1.05);
}

.telegram-widget-wrapper:focus-within .telegram-overlay {
  outline: none;
  ring: 2px;
  ring-color: #1e90ff;
}

/* Hide the Telegram widget but keep it functional */
...
.telegram-widget-hidden :deep(iframe) {
  display: block;
  margin: 0 auto;
  width: 100% !important;
  max-width: 100%;
