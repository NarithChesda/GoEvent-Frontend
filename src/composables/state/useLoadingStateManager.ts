...
export function useLoadingStateManager() {
  ...
  const completedLoading = ref<Map<string, { state: LoadingState, endTime: number, success: boolean }>>(new Map())
   ...
   */
  const createDependencyAwareLoader = <T>(
    context: string,
    dependencies: string[] = []
  ) => {
  ...
  const getLoadingByContext = (context: string) => {
    return Array.from(activeLoading.value.values()).filter(state => 
  ...
  const isLoading = (context?: string): boolean => {
    ...
    return Array.from(activeLoading.value.values()).some(state => 
      state.context === context
    )
  }
   ...
   */
  const cleanup = (olderThanMs: number = 5 * 60 * 1000) => { // 5 minutes default
