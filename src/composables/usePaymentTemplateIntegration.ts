...
export function usePaymentTemplateIntegration(event: Event) {
  ...
  const confirmedPayments = computed(() => {
    return payments.value.filter((p) => p.status === 'confirmed' && p.event === event.id)
  })

  const pendingPayments = computed(() => {
    return payments.value.filter((p) => p.status === 'pending' && p.event === event.id)
  })
  ...
  const getPaymentForTemplate = (templateName: string): Payment | null => {
    return (
      payments.value.find(
        (payment) => payment.event === event.id && payment.template_name === templateName,
      ) || null
    )
  }
