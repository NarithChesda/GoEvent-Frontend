...
- **Drawers, not modals, for create/edit flows**: full-screen sheet on mobile, floating right panel on desktop, brand-gradient header, sticky footer with gradient submit. A create flow that asks one question at a time is the same drawer with a step wizard inside — progress segments in the header, Back left / Continue right in the footer (§10 of the design skill, reference [EventCreateDrawer.vue](src/components/EventCreateDrawer.vue)). Modals are reserved for confirmations and pickers (`rounded-3xl`, centered, destructive confirms use the red icon-disc layout). **Inside a drawer, a picker discloses in place rather than opening its own sheet** — a sheet at `z-[1001]` over a drawer at `z-[999]` is a modal over a modal. Settings-style controls stack in a grouped inset list (§8 of the design skill), and a two-or-three-way named choice is a segmented control, never a switch whose label flips with its state.
