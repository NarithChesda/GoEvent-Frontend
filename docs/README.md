...
- [cover-details.md](backend-api-requirements/cover-details.md) — the cover's hosts / date / venue blocks: `showCoverHosts`/`showCoverDate`/`showCoverLocation`/`showCoverGuestName` + a `coverDetails` config inside `cover_stage_layout`, three more `coverElements` keys, and one new image field, `cover_host_separator_image`
- [cover-photo-frame.md](backend-api-requirements/cover-photo-frame.md) — the cover's photo frame: `is_cover_photo` on EventPhoto (which photo fills it, framed by its existing `crop_*`), and two template images, `cover_photo_frame_image` and `cover_photo_shape_image`, replacing the `sample_logo_1`/`sample_logo_2` pair. `showCoverPhoto`, `coverPhoto` and `coverElements.photo` ride in the existing `cover_stage_layout` blob
- [showcase-animation-stack.md](backend-api-requirements/showcase-animation-stack.md): a third `showcaseAnimationType` value, `stack` (the photo-stack transition), plus a `stackLayout` key picking its composition (`pile`/`split`/`booth`/`mosaic`/`film`), both inside the same `cover_stage_layout` blob. No new field; only check that the serializer doesn't allow-list the blob's keys or values
...
- [text-effects.md](backend-api-requirements/text-effects.md) — nullable `text_effects` JSON on the event template: a metallic finish (`foil`/`relief` in `gold`/`rose_gold`/`silver`) per font slot, gilding that slot's display text — cover header and guest name, the Save the Date, host names, section headings — on every V1 stage. Absent means no finish, so the frontend ships inert and nothing needs backfilling
