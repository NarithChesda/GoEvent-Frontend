...
- The **catalogue** `GET /api/payment/credit-packs/` is public — no token, no `403`. Sending a token widens it: a partner also sees their `is_public: false` packs. `is_featured` is **presentation only** (the "most popular" highlight); it never filters, prices or reorders anything — position is `display_order`. The public partner page ([usePartnerPricingTiers.ts](src/composables/usePartnerPricingTiers.ts)) renders two authored lead cards (free trial, pay-as-you-go) and then the public packs
