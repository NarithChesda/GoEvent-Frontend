...
- **Per-stage modes** (`template_assets.stage_modes`, resolved by `resolveStageModes` in [useStageModes.ts](src/composables/showcase/useStageModes.ts)): which of the three V1 stages is built from **artwork** and which plays a **film** — declared by the template, one key per stage (`cover`, `transition`, `background`), each `animation` or `video`. One vocabulary for all three, because it is one question asked three times. Replaces the inference that read a *package plan* out of the uploaded assets: `standard_cover_video` being present used to switch the cover, the middle beat **and** the invitation's backdrop to video together, and the animated middle beat was additionally hard-limited to `category === 'wedding'`. Neither is the template's intent, and neither could be changed except by uploading or deleting a video. Backend field is **pending**: [docs/backend-api-requirements/stage-modes.md](docs/backend-api-requirements/stage-modes.md)
  ...
  - **There is no `none` mode.** "No middle beat" is not a third shape, it is what `animation` degrades to when the event has no featured photo, and what `video` degrades to when there is no film — so it needs no vocabulary of its own, and a partner can't accidentally pick a stage that renders nothing.
  ...
  - **The form's pickers have no "leave it to the system" option**, unlike `save_the_date_design`'s `auto`. They are seeded from `resolveStageModes` on the template's own assets, so they open on what the template already renders and saving pins exactly that; all three keys are then always persisted.
