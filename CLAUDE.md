...
- **A sample invitation instead of an event.** [demo-showcase-event.json](src/assets/demo-showcase-event.json) holds one wedding shaped exactly like a `GET /api/events/:id/showcase/` response, with every optional section populated — a partner judging a template has to see what it does to a host row, a two-day agenda, a gallery, a payment card and a dress code. [useDemoShowcase.ts](src/composables/showcase-preview/useDemoShowcase.ts) serves it **like the endpoint does**: one call, one language. That is what lets it plug into `useEventShowcase`'s new `dataSource` option, leaving every merge, font pass and in-place language switch downstream untouched. Dates carry `{year}`, rolled forward so the countdown never expires; media lives in `public/demo-showcase/` and is resolved to absolute URLs so `getMediaUrl` leaves it alone.
- **Its own frame route**, [TemplateShowcasePreviewFrameView.vue](src/views/TemplateShowcasePreviewFrameView.vue) at `/template-showcase-preview-frame?stage=&templateId=&lang=`. Separate from `ShowcasePreviewFrameView` because that one takes an event id, fetches that event, and carries the whole inline-editing apparatus for its owner. **The path keeps the `showcase-preview-frame` segment deliberately** — that substring is what `isPreviewFrameDocument()` matches, so these iframes skip app-shell startup for free.
...
**Switching templates never touches an iframe's `src`.** The page fetches the chosen template's assets once and pushes them to every frame over the bridge (`preview-template` — the same live try-on `BrowseTemplateModal` uses); the language switch is `set-language` for the same reason. Changing `src` re-navigates, and with three frames that is three full app boots per click. Frames are mounted one at a time — on a phone, only the stage actually on screen, the rest on demand.
